...
pub async fn cmd_share_file(
    ...
    if client_opt.is_none() { 
        ...
        let mock_link = format!("https://t.me/c/{}/{}", folder_id.unwrap_or(0).abs(), message_id);
        log::info!("[MOCK] Generated share link: {}", mock_link);
        return Ok(mock_link);
    }
    
    // For real Telegram channels, we need to generate the public link
    // Format: https://t.me/c/channel_id/message_id
    // Note: channel_id needs to be positive for the link format
    let channel_id = match folder_id {
        Some(id) => id.abs(),
        None => return Err("No folder specified".to_string()),
    };
    
    let share_link = format!("https://t.me/c/{}/{}", channel_id, message_id);
    log::info!("Generated share link for message {} in channel {}: {}", message_id, channel_id, share_link);
...
pub async fn cmd_rename_file(
    ...
    if client_opt.is_none() { 
        ...
        return Ok("File renamed successfully".to_string());
    }
    ...
    if let Media::Document(ref doc) = media {
        ...
        log::info!("Downloading file for rename: {}", doc.name());
        ...
        let upload_result = async {
            ...
            // Send the new message with same text as original
            let new_message = InputMessage::new().text(text).file(uploaded_file);
        ...
        // Only delete original after successful upload
        client.delete_messages(&peer, &[message_id]).await.map_err(|e| e.to_string())?;
        
        log::info!("Renamed message {} to {}", message_id, new_name);
        Ok("File renamed successfully".to_string())
    } else {
        Err("Can only rename document files".to_string())
    }
}
