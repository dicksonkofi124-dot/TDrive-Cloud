...
export function ContextMenu({ x, y, file, onClose, onDownload, onDelete, onPreview, onShare, onRename }: ContextMenuProps) {
    ...
    return (
        ...
        >
            ...
            {file.type !== 'folder' && onShare && (
                <button onClick={() => { onShare(file); onClose(); }} className="flex items-center gap-2 px-2 py-1.5 text-sm text-telegram-text hover:bg-telegram-hover rounded transition-colors text-left w-full">
                    <Share2 className="w-4 h-4 text-blue-500" />
                    Share
                </button>
            )}

            <button onClick={() => { onDownload(file); onClose(); }} className="flex items-center gap-2 px-2 py-1.5 text-sm text-telegram-text hover:bg-telegram-hover rounded transition-colors text-left w-full">
                <HardDrive className="w-4 h-4 text-green-500" />
                Open Download Manager
            </button>

            {onRename && (
                <button onClick={() => { onRename(file); onClose(); }} className="flex items-center gap-2 px-2 py-1.5 text-sm text-telegram-text hover:bg-telegram-hover rounded transition-colors text-left w-full">
