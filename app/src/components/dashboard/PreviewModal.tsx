...
import { X, File, ChevronLeft, ChevronRight, Share2 } from 'lucide-react';
...
export function PreviewModal({ file, onClose, onNext, onPrev, onShare, currentIndex, totalItems, nextFile, prevFile, activeFolderId }: PreviewModalProps) {
        ...
        <div className="fixed inset-0 z-[150] bg-black/90 flex items-center justify-center p-4 backdrop-blur-sm" onClick={onClose}>
            <div className="relative max-w-5xl w-full max-h-screen flex flex-col items-center justify-center" onClick={e => e.stopPropagation()}>
                ...
                {onShare && (
                    <button
                        onClick={onShare}
                        className="absolute -top-12 right-12 p-2 bg-black/60 hover:bg-black/80 rounded-full transition-colors"
                        style={{ color: '#ffffff' }}
                        title="Share file"
                    >
